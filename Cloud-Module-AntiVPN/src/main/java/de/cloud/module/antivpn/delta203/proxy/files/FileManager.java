...
package de.cloud.module.antivpn.delta203.proxy.files;
...
public class FileManager {
  ...
  public FileManager(String filename) {
    ...
    file = new File(AntiVPN.plugin.getDataFolder(), filename);
  }
   ...
   */
  public void create() {
    if (!AntiVPN.plugin.getDataFolder().exists()) {
      AntiVPN.plugin.getDataFolder().mkdir();
    }
    try {
      if (!file.exists()) {
        Files.copy(
            Objects.requireNonNull(AntiVPN.plugin.getResourceAsStream(filename)), file.toPath());
      }
